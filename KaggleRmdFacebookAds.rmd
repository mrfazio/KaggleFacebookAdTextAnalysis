...
```{r}
political_msgs <- paste((df_ads %>% filter(political > not_political))$message, collapse = ' ')
not_political_msgs <- paste((df_ads %>% filter(political < not_political))$message, collapse = ' ')

political_corpus <- generate_corpus(political_msgs)
not_political_corpus <- generate_corpus(not_political_msgs)
```

```{r}
get_tf_idf_df <- function(corpus, n_top = 100) {
  docTermMat <- DocumentTermMatrix(corpus, control = list(weighting = weightTfIdf))
  matTfIdf <- as.matrix(findMostFreqTerms(docTermMat, n = n_top))
  flat_matTfIdf <- flatten(matTfIdf)
  df_words_tfidf <- data.frame(Word = names(flat_matTfIdf), TfIdf = unlist(flat_matTfIdf))
  # df_words_tfidf <- unique(df_words_tfidf) %>% arrange(desc(TfIdf))
  return(df_words_tfidf)
}

df_tfidf_political <- get_tf_idf_df(political_corpus)
df_tfidf_not_political <- get_tf_idf_df(not_political_corpus)
```
